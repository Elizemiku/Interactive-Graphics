...
highchart() %>% 
  ...
  hc_add_series(dat, type = "scatter",  hcaes(x = xvar, y = yvar, group = cond),  maxSize = 5) %>% 
  hc_add_series(fit, type = "arearange", hcaes(x = xvar,
                   low = .fitted + .se.fit,
                   high = .fitted -  .se.fit))
...
highchart() %>% 
  ...
  hc_add_series(points, type = "scatter", hcaes(x = x1, y = x2, group = cluster)))
